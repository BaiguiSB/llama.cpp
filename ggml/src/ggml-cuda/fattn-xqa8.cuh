...
// The body lives in namespace fattn_xqa8 so fattn.cu can include this header
// next to fattn-xqa.cuh without symbol collisions. Not re-indented on purpose:
// diff against fattn-xqa.cuh must show only the GQA-8 edits, so fixes to the
// validated GQA-6 kernel can be mirrored here mechanically.
namespace fattn_xqa8 {

// Specialized FlashAttention decode kernel for Volta, Qwen3.6-35B-A3B only:
// head_dim 256, GQA ratio 8 (16 Q heads on 2 KV heads), q8_0 K and V.
// One block serves one (KV head, KV split) and computes all 8*n_tokens Q rows
// on its split, so the KV of the split is read exactly once. This removes the
// 8:1 GQA read redundancy of the vector kernel; the tile/mma fallbacks for
// these shapes pay a full f16 staging round trip instead (no q8 tile instance
// packs 8 rows).
...
// The M = 8 wmma tile holds the 8*n_tokens Q rows; with GQA 8 every tile is
// exactly full (NT == n_tokens), the tail zero-padding of the original is dead.
...
template<int NT> // NT == ceil(8*n_tokens/8) == n_tokens, number of M = 8 Q tiles
// NT=1 fits 2 blocks/SM (2*46.5 KB smem <= 96 KB), NT>=2 needs 57.0/67.5/78.0 KB
// (NT=2/3/4) and is smem-limited to 1 block/SM anyway. Promising 2 blocks there
// only capped ptxas at 128 registers without any occupancy gain, which spilled
// registers once the NT>=2 row loops actually executed. NT=4 is new relative to
// the GQA-6 kernel; spill status must be confirmed with cuobjdump (REG <= 255,
// STACK/LDL/STL == 0 under the (256,1) bounds).
...
static __global__ void flash_attn_ext_xqa(
    ...
    // Q rows are head-major, r = qh*n_tokens + tok. With GQA 8 every M tile is
    // exactly full (rows == 8*NT), the zero-padding loop below never runs.
    const int q_head0 = kv_head*8;
            ...
            for (int j = 0; j < TILE*(PANEL/16)/fattn_xqa_nthreads; ++j) {
                ...
                if (i < nslices) {
                    ...
                    if (use_pf && j < fattn_xqa_pf_slices) {
                        // Qualified: ADL on the half* argument would add the global twin
                        // from fattn-xqa.cuh as a candidate in TUs including both -> ambiguous.
                        fattn_xqa8::dequantize_V_q8_0_regs(pf_qs[j], pf_d[j], &sKV[t][sl*16]);
                    } else {

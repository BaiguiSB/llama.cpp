...
// Single source of truth for the XQA tensor-core path, consulted by both the
// routing in ggml_cuda_get_best_fattn_kernel and ggml_cuda_flash_attn_ext_get_alloc_size.
// Only the Qwen3.8-27B decode shapes are supported, everything else falls back
// to the regular kernel selection.
bool ggml_cuda_fattn_xqa_supported(const int device, const ggml_tensor * dst) {
    ...
    if (Q->ne[2] % K->ne[2] != 0 || Q->ne[2] / K->ne[2] != 6) {
        return false; // GROUP_SIZE 6 is built into the kernel
    }

    if (Q->ne[1] < 1 || Q->ne[1] > 4) {
        return false; // rows = 6*n_tokens <= 24 = 3 M tiles
    }
